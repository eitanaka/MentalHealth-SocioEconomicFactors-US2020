...
We have ade used of 10 different datasets and the one which we are going to use from the previous project. 
...
<!-- Planning Database with 2010 Census and 2014 – 2018 American Community SurveyData: We imported the dataset from the shared drive and have cleaned up dataset and made used of some variables -->
...
outlier_df <- outlier_df[,1:(ncol(outlier_df)-1)]
```
...
par(mfrow=c(5, 10))
for (i in 6:ncol(outlier_df)) {
  boxplot(outlier_df[[i]], main=names(outlier_df)[i])
}
```
...
for (col in names(cols_of_interest)) {
  if (col != "DEPRESSION") {
    plot_data <- data.frame(x = outlier_df$DEPRESSION, y = outlier_df[[col]])
...
for (col in names(cols_of_interest)) {
  if (col != "MHLTH") {
    plot_data <- data.frame(x = outlier_df$MHLTH, y = outlier_df[[col]])
    plot <- ggplot(plot_data, aes(x = x, y = y)) + 
      geom_point() +
      geom_smooth(method = "lm", se = FALSE) +
      xlab("mhlth") +
...
### Correlations
```{r}
cor_matrix <- cor(outlier_df[,6:ncol(outlier_df)])

# Create 6 subset for cor_matrix or cor_heatmap
dep_mth_df <- subset(outlier_df, select = "DEPRESSION")
dep_mth_df <- cbind(dep_mth_df, outlier_df[,9])

# Data frame containing depression, mhlth, health risk behaviors
dep_mth_hrb <- dep_mth_df
dep_mth_hrb["BINGE"] <- outlier_df[,6]
dep_mth_hrb["LPA"] <- outlier_df[,8]
dep_mth_hrb["SLEEP"] <- outlier_df[,10]

# Data frame containing depression, mhlth, MT
dep_mth_mt <- cbind(dep_mth_df, outlier_df[12:16])
# Data frame containing depression, mhlth, EA, MI, Pop
dep_mth_ea_mi_pop <- cbind(dep_mth_df, outlier_df[18:24])
# Data Frame containing depression, mhlth, MI, Pop, CT
dep_mth_ct <- cbind(dep_mth_df, outlier_df[26:34])
# Data Frame containing depression, mhlth, ES 
dep_mth_es <-cbind(dep_mth_df, outlier_df[36:41])
# Data Frame containing depression, mhlth, others
dep_mth_geo <- cbind(dep_mth_df, outlier_df[42:length(master_df)])
...
plot(depression.tree, uniform=TRUE, main="Depression Tree")
...
plot(mhlth.tree, uniform=TRUE, main="Poor Mental Health Tree")
