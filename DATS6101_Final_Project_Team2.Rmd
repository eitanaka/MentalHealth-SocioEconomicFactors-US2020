...
```{r}
# Evaluate model with cross-validation and calculate metrics
# R-square
# MSE
# RMSE
# Cross-Validation
...
train.mhlth.tree <- rpart(MHLTH ~ ACCESS2 + BINGE + CHECKUP + DIABETES + LPA + OBESITY + PHLTH + SLEEP + STROKE + mt.nev.mar + mt.now.mar + MT_Divorces + MT_Separated + MT_Widowed + ea.less.hs.deg + ea.hs.deg + ea.col.ass.deg + ea.ba.deg + ea.grad.prof.deg + MI_Estimate + tot.pop + `CT_<10` + `CT_10-14` + `CT_15-19` + `CT_20-24` + `CT_25-29` + `CT_30-34` + `CT_35-44` + `CT_45-59` + `CT_>60` + ES_Total_labor_force + ES_Civilian_labor_force + ES_Civilian_labor_force_employed + ES_Civilian_labor_force_unemployed + ES_Armed_Forces + ES_Not_in_labor_force + land + urban + poverty + no.ins + disab + no.comp + `broad&comp` + no.eng + sing.mom + live.alone + pub.assist + no.phone + no.plumb + married.kid + hhd.no.comp + hhd.only.phone + hhd.no.int + hhd.broad + index_apr20, data = test.set, control = rpart.control(maxdepth = ))
...
# The RMSE (Root Mean Squared Error) of 2.46 is the square root of the MSE, and it represents the average deviation of the predicted values from the actual values, with a lower value indicating better performance. Overall, the values suggest that the regression tree model has some predictive power, but there is still room for improvement.
...
Hyperparameter tuning: Most algorithms have hyperparameters that need to be tuned to obtain the best performance. You can use techniques such as grid search, random search, or Bayesian optimization to find the optimal hyperparameters for your model.
```{r}
# R-square
# MSE
# RMSE
...
#### Multiple Linear Regression Model vs. Regression Tree model
```{r}
