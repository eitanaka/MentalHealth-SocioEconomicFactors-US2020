...
for (col_name in colnames(num_df)) {
  # remove outliers
  new_num_df <- outlierKD2(num_df, num_df[[col_name]], rm=TRUE)
...
numeric_vars <- sapply(new_num_df, is.numeric)
num_df <- new_num_df[, numeric_vars]
...
dep_hist <- hist(new_num_df$DEPRESSION, main = "Distribution of Tract-level Depression Rates")
dep_hist
mhlth_hist <- hist(new_num_df$MHLTH, main = "Distribution of Tract-level Poor Mental Health Rates")
mhlth_hist
```
...
dep_qq <- qqnorm(new_num_df$DEPRESSION, main = "Distribution of Tract-level Depression Rates")
dep_qq
mhlth_qq <- qqnorm(new_num_df$MHLTH, main = "Distribution of Tract-level Poor Mental Health Rates")
mhlth_qq
```
