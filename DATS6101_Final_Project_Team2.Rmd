...
Our depression rate regression tree has an R-squared value of 0.348 and a mean squared error (MSE) of 6.12 (representing the average squared difference between the predicted values and the actual values, with a higher value indicating worse performance). This indicates that approximately 35 percent of the variation in the depression rate dependent variable is associated with variation in the independent variables. On the other hand, our mental health rate tree has an R-squared value of 0.584 and an MSE of 1.35. So in comparison to one another, the mental health regression tree performs better. 
...
After running the testing data through both our regression trees (created from the training data) we get a more accurate sense of the utility of our models. For our depression tree, we have an R-squared value of 0.399, suggesting that the regression tree model explains 39.9% of the variation in the data, which is a moderate level of explanation. It also has a MSE of 6.05. For our poor mental health tree, we have an R-squared value of 0.685, indicating that 68.5% of the variability in the MHLTH variable can be explained by the model. The MSE is 1.07, which means that on average, the model's predictions are off by 1.07 units from the actual values. The root mean squared error (RMSE) is 1.04, which is the same as the standard deviation of the residuals. This indicates that the model's predictions have a standard deviation of 1.04 around the actual values. Overall, these metrics suggest that the model has a moderate level of predictive power for the MHLTH variable.
