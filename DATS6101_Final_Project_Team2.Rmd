...
Need edited
```{r}
cols_depression <- names(which(abs(cor_matrix[2, ]) >= 0.3))
cols_names_depression <- c("var1", cols_depression)

cols_mhlth <- names(which(abs(cor_matrix[4, ]) >= 0.3))

cols_mhlth <- cbind(master_df[,])

mhlth_cor_df <- subset(master_df, select = cols_mhlth)
```
